...
public class BlockTubeTurbo extends Block implements IConnectable {
    public static BlockTubeTurbo instance;
    ConnectedTextures textures[] = new ConnectedTextures[12];

    public BlockTubeTurbo(String n) {
        super(Material.rock);
        setBlockName(n);
        instance = this;
        setLightOpacity(0);
        ...
        textures[0] = new ConnectedTextures(TubeTransportSystem.MOD_ID + ":tube/%s", this, 0);//tube0
        textures[1] = new ConnectedTextures(TubeTransportSystem.MOD_ID + ":tube/%s", this, 1);//1
        textures[2] = new ConnectedTextures(TubeTransportSystem.MOD_ID + ":tube/%s", this, 2);//2
        textures[3] = new ConnectedTextures(TubeTransportSystem.MOD_ID + ":tube/%s", this, 3);//3
        textures[4] = new ConnectedTextures(TubeTransportSystem.MOD_ID + ":tube/%s", this, 4);//4
        textures[5] = new ConnectedTextures(TubeTransportSystem.MOD_ID + ":tube/%s", this, 5);//5
    ...
    public IIcon getIcon(IBlockAccess blockAccess, int x, int y, int z, int s) {
        ...
        if (meta <= 5) {
            ...
            if ((d == ForgeDirection.UP || d == ForgeDirection.DOWN) && (s == 0 || s == 1))
                return RenderTubeTurbo.IS_INTERNAL ? textures[meta + 6].getIconForSideForInternal(blockAccess, x, y, z, s) : textures[meta + 6].getIconForSide(blockAccess, x, y, z, s);
            else if  ((d == ForgeDirection.NORTH || d == ForgeDirection.SOUTH) && (s == 2 || s == 3))
                return RenderTubeTurbo.IS_INTERNAL ? textures[meta + 6].getIconForSideForInternal(blockAccess, x, y, z, s) : textures[meta + 6].getIconForSide(blockAccess, x, y, z, s);
            else if  ((d == ForgeDirection.EAST || d == ForgeDirection.WEST) && (s == 4 || s == 5))
                    return RenderTubeTurbo.IS_INTERNAL ? textures[meta + 6].getIconForSideForInternal(blockAccess, x, y, z, s) : textures[meta + 6].getIconForSide(blockAccess, x, y, z, s);

            return RenderTubeTurbo.IS_INTERNAL ? textures[meta].getIconForSideForInternal(blockAccess, x, y, z, s) : textures[meta].getIconForSide(blockAccess, x, y, z, s);
        }
    ...
    public void onEntityCollidedWithBlock(World world, int x, int y, int z, Entity entity) {
        ...
        Utilities.entityAccelerateTurbo(entity, ForgeDirection.getOrientation(world.getBlockMetadata(x, y, z)));
        Utilities.entityLimitSpeedTurbo(entity);
