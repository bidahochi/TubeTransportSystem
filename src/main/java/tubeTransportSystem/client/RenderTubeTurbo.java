...
public class RenderTubeTurbo implements ISimpleBlockRenderingHandler {
    public static int ID;
    public static boolean IS_INTERNAL = false;
    
    @Override
    public void renderInventoryBlock(Block block, int metadata, int modelId, RenderBlocks renderer) {
        renderer.setOverrideBlockTexture(ItemTubeTurbo.instance.getIconFromDamage(metadata));
    ...
    public boolean renderWorldBlock(IBlockAccess world, int x, int y, int z, Block block, int modelId, RenderBlocks renderer) {
        ...
        for (int i = 0; i < 6; i++) {
            ...
            renderBound[i] = BlockTubeTurbo.instance.canConnectTo(world, x, y, z, d);
        }
        ...
        rendered = renderer.renderStandardBlock(BlockTubeTurbo.instance, x, y, z);
